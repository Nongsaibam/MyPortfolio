...
const ProfilePage = () => {
  ...
  const customRunningStyle = siteSettings?.enableRunningGradientText !== false ? {
    backgroundImage: `linear-gradient(90deg, ${siteSettings?.runningGradientColor1 || '#38bdf8'}, ${siteSettings?.runningGradientColor2 || '#818cf8'}, ${siteSettings?.runningGradientColor3 || '#c084fc'}, ${siteSettings?.runningGradientColor1 || '#38bdf8'})`,
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-between overflow-hidden bg-transparent text-slate-900 dark:text-white select-none">
      <SEO />

      {/* Floating Navbar */}
      <header className="fixed top-6 left-0 right-0 z-50 mx-auto max-w-5xl px-4 sm:px-6">
        <div className="flex items-center justify-between rounded-full border border-black/10 bg-white/70 px-4 py-2.5 backdrop-blur-2xl shadow-xl transition-all duration-300 dark:border-white/15 dark:bg-slate-900/80">
          <Link to="/" className="flex items-center gap-2.5 font-black text-lg tracking-wider">
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-500/20 text-sm">
              TK
            </span>
            <span className="hidden sm:inline-block font-extrabold text-slate-900 dark:text-white">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navItems.map((item) => (
              <a
                ...
                className="text-xs font-semibold uppercase tracking-wider text-slate-600 hover:text-cyan-500 dark:text-slate-300 dark:hover:text-cyan-400 transition"
              >
                {item}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              onClick={toggleTheme}
              className="flex h-9 w-9 items-center justify-center rounded-full border border-black/10 bg-white/50 text-slate-700 backdrop-blur-xl transition hover:scale-105 dark:border-white/15 dark:bg-white/10 dark:text-amber-300"
            ...
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-full border border-black/10 bg-white/50 text-slate-700 backdrop-blur-xl md:hidden dark:border-white/15 dark:bg-white/10 dark:text-white"
            >
        ...
        {isMobileMenuOpen && (
          <div className="mt-3 rounded-2xl border border-black/10 bg-white/90 p-4 backdrop-blur-2xl shadow-2xl md:hidden dark:border-white/15 dark:bg-slate-900/95 animate-fade-down">
            <nav className="flex flex-col gap-3">
              {navItems.map((item) => (
                <a
                  ...
                  className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/10"
                >
                  {item}
                </a>
              ))}
      ...
      >
        <span className="mb-1 text-[10px] font-bold uppercase tracking-[0.25em] text-slate-400 dark:text-slate-500 transition-colors duration-300 hover:text-cyan-400">
          Scroll Down
        </span>
        <div className="flex h-8 w-8 items-center justify-center rounded-full border border-black/10 bg-white/50 text-slate-600 backdrop-blur-xl shadow-sm transition-all duration-300 hover:border-cyan-400 hover:bg-cyan-500 hover:text-white dark:border-white/15 dark:bg-white/10 dark:text-slate-300">
