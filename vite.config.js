...
try {
  console.log("Staging full profile image fit updates...");
  execSync("git add .", { cwd: process.cwd(), encoding: "utf-8" });

  console.log("Committing updates...");
  try {
    const commitRes = execSync('git commit -m "Update profile avatar image to object-contain so entire image and head are fully visible without cropping"', { cwd: process.cwd(), encoding: "utf-8" });
