...
try {
  console.log("Staging new navbar design updates...");
  execSync("git add .", { cwd: process.cwd(), encoding: "utf-8" });

  console.log("Committing updates...");
  try {
    const commitRes = execSync('git commit -m "Update navbar design with dual-tone TK gradient logo, centered nav links, and Light/Dark pill button matching user reference screenshot"', { cwd: process.cwd(), encoding: "utf-8" });
